...
void main() async {
  ...
  for (final file in files) {
    ...
    final regexConstTheme = RegExp(
      r'const\s+([A-Za-z0-9_]+)\([^)]*Theme\.of\(context\)',
    );
    while (regexConstTheme.hasMatch(content)) {
      ...
      content =
          ...
          content.substring(start + 5); // remove 'const'
      changed = true;
    }

    final regexConstTheme2 = RegExp(
      r'const\s+([A-Za-z0-9_]+)\([^)]*Theme\.of\(context\)[^)]*\)',
    );
    ...
    if (content.contains('const BoxDecoration(color: Theme.of(context)')) {
      content = content.replaceAll(
        'const BoxDecoration(color: Theme.of(context)',
    ...
    if (content.contains(
      'const Icon(Icons.search_off, size: 64, color: Theme.of(context)',
    )) {
      content = content.replaceAll(
        'const Icon(Icons.search_off, size: 64, color: Theme.of(context)',
        'Icon(Icons.search_off, size: 64, color: Theme.of(context)',
      );
      changed = true;
    }

    if (content.contains(
      'const CircularProgressIndicator(color: Theme.of(context)',
    )) {
      content = content.replaceAll(
        'const CircularProgressIndicator(color: Theme.of(context)',
